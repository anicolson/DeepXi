...
test_epoch = 20 # epoch for testing.
test_noisy_speech_ver = 'v2' # test noisy speech set version.
test_noisy_speech_path = set_path + '/DeepXi/test_' + test_noisy_speech_ver + '/noisy_speech'
...
bidi = None # use a Bidirectional Recurrent Neural Network.
bidi_con = None # forward and backward cell activation connection.
...
stats_path = '/home/aaron/aaron/Dropbox/DeepXi/stats'
mu_mat = spio.loadmat(stats_path + '/clean_speech_' + train_clean_speech_ver + 
	'_noise_' + train_noise_ver + '_mu.mat') # mean of a priori SNR in dB from MATLAB.
mu = tf.constant(mu_mat['mu'], dtype=tf.float32) 
sigma_mat = spio.loadmat(stats_path + '/clean_speech_' + train_clean_speech_ver + 
	'_noise_' + train_noise_ver + '_sigma.mat') # standard deviation of a priori SNR in dB from MATLAB.
...
if train or val:
	...
	val_clean_speech, val_clean_speech_len, val_snr, _ = batch._test_set(val_clean_speech_path, '*.wav', train_snr_list) # clean validation waveforms and lengths.
	val_noise, val_noise_len, _, _ = batch._test_set(val_noise_path, '*.wav', train_snr_list) # noise validation waveforms and lengths.

## TEST NOISY SPEECH SET
if test: test_noisy_speech, test_noisy_speech_len, test_snr, test_fnames = batch._test_set(test_noisy_speech_path, '*.wav', [0]) # noisy speech test waveforms and lengths.
...
if test:
	...
	with tf.Session(config=config) as sess:
		...
		for j in range(len(test_noisy_speech_len)):
			...
			elif gain is 'mmse-lsa': gain_out = mmse_stsa(xi_hat_out, ml_gamma_hat(xi_hat_out)) # MMSE-LSA estimator gain.
