...
parser.add_argument('--ver', default='c2.7a', type=str, help='Model version')
...
parser.add_argument('--test_epoch', default=81, type=int, help='Epoch for testing')
...
## PATHS - AARON
# parser.add_argument('--model_path', default='/media/aaron/Filesystem/model/DeepXi', type=str, help='Model save path')
# parser.add_argument('--set_path', default='/home/aaron/set', type=str, help='Path to datasets')
# parser.add_argument('--stats_path', default='/home/aaron/Dropbox/DeepXi/stats', 
# 	type=str, help='Path to training set statistics')
# parser.add_argument('--test_noisy_speech_path', default='/home/aaron/recon_2018/noisy_speech', 
# 	type=str, help='Path to the noisy speech test set')
# parser.add_argument('--out_path', default='/media/aaron/Filesystem/out/recon_2018', 
# 	type=str, help='Output path')

## PATHS - PINKY
# model_path = '/home/aaron/model/DeepXi/' + version # model save path.
# set_path = '/home/aaron/set'
# stats_path = '/home/aaron/aaron/home/aaron/Dropbox/DeepXi/stats'
# train_clean_speech_path = set_path + '/DeepXi/train_' + train_clean_speech_ver + '/train_clean_speech' # path to the clean speech training set.
# train_noise_path = set_path + '/DeepXi/train_noise_' + train_noise_ver # path to the clean speech training set.
# val_clean_speech_path = set_path + '/DeepXi/train_' + train_clean_speech_ver + '/val_clean_speech' # path to the clean speech validation set.
# val_noise_path = set_path + '/DeepXi/train_' + train_clean_speech_ver + '/val_noise' # path to the noise validation set.
# test_noisy_speech_path = ''
# out_path = '' + version + '/' + 'e' + str(test_epoch)

## PATHS - FIST
parser.add_argument('--model_path', default='/home/aaron/model/DeepXi', type=str, help='Model save path')
parser.add_argument('--set_path', default='/media/aaron/SSD', type=str, help='Path to datasets')
parser.add_argument('--stats_path', default='/home/aaron/aaron/Dropbox/DeepXi/stats', 
	type=str, help='Path to training set statistics')
parser.add_argument('--test_noisy_speech_path', default='/home/aaron/aaron/set/DeepXi/test_v2/noisy_speech', 
	type=str, help='Path to the noisy speech test set')
parser.add_argument('--out_path', default='/home/aaron/aaron_root/media/aaron/Filesystem/out/DeepXi/test_v2', 
	type=str, help='Output path')

## PATHS - N060
# parser.add_argument('--model_path', default='/export/home/s2842809/scratch/home/model/DeepXi', type=str, help='Model save path')
# parser.add_argument('--set_path', default='/lscratch/s2842809/set', type=str, help='Path to datasets')
# parser.add_argument('--stats_path', default='/export/home/s2842809/scratch/home/DeepXi/stats', 
# 	type=str, help='Path to training set statistics')
# parser.add_argument('--test_noisy_speech_path', default='', 
# 	type=str, help='Path to the noisy speech test set')
# parser.add_argument('--out_path', default='', 
# 	type=str, help='Output path')

## NETWORK PARAMETERS
parser.add_argument('--blocks', default=['C3'] + ['B5']*40 + ['O1'], type=list, help='Residual blocks')
parser.add_argument('--cell_size', default=257, type=int, help='Cell size')
...
parser.add_argument('--peep', default=None, type=bool, help='Use peephole connections')
parser.add_argument('--bidi', default=None, type=bool, help='Bidirectional recurrent neural network flag')
...
parser.add_argument('--coup_conv_filt', default=257, type=int, help='Number of filters for coupling unit')
...
parser.add_argument('--conv_caus', default=True, type=bool, help='Causal convolution flag')
...
args.train_clean_speech_path = args.set_path + '/DeepXi/train_' + args.train_clean_speech_ver + '/train_clean_speech' # path to the clean speech training set.
args.train_noise_path = args.set_path + '/DeepXi/train_noise_' + args.train_noise_ver # path to the clean speech training set.
args.val_clean_speech_path = args.set_path + '/DeepXi/train_' + args.train_clean_speech_ver + '/val_clean_speech' # path to the clean speech validation set.
args.val_noise_path = args.set_path + '/DeepXi/train_' + args.train_clean_speech_ver + '/val_noise' # path to the noise validation set.
...
mu_mat = spio.loadmat(args.stats_path + '/clean_speech_' + args.train_clean_speech_ver + 
	'_noise_' + args.train_noise_ver + '_sample_size_' + str(args.sample_size) + '_mu_xi.mat') # mean of a priori SNR in dB from MATLAB.
mu = tf.constant(mu_mat['mu_xi'], dtype=tf.float32) 
sigma_mat = spio.loadmat(args.stats_path + '/clean_speech_' + args.train_clean_speech_ver + 
	'_noise_' + args.train_noise_ver + '_sample_size_' + str(args.sample_size) + '_sigma_xi.mat') # standard deviation of a priori SNR in dB from MATLAB.
...
if args.test:
	...
	## ENHANCEMENT
	if args.gain is 'ibm': G = tf.cast(tf.greater(xi_hat_ph, 1), tf.float32) # IBM gain function.
	if args.gain is 'wf': G = tf.div(xi_hat_ph, tf.add(xi_hat_ph, 1.0)) # WF gain function.
	if args.gain is 'srwf': G = tf.sqrt(tf.div(xi_hat_ph, tf.add(xi_hat_ph, 1.0))) # SRWF gain function.
	if args.gain is 'cwf':
		G = tf.sqrt(xi_hat_ph)
	...
	with tf.Session(config=config) as sess:
		...
		for j in range(len(test_noisy_speech_len)):
			...
			if args.gain is 'mmse-stsa': gain_out = mmse_stsa(xi_hat_out, ml_gamma_hat(xi_hat_out)) # MMSE-STSA estimator gain.
			elif args.gain is 'mmse-lsa': gain_out = mmse_lsa(xi_hat_out, ml_gamma_hat(xi_hat_out)) # MMSE-LSA estimator gain.
			else: gain_out = sess.run(G, feed_dict={xi_hat_ph: xi_hat_out}) # gain.	

			if args.out_type is 'raw': # raw outputs from network (.mat).
			...
			if args.out_type is 'xi_hat': # a priori SNR estimate output (.mat).
			...
			if args.out_type is 'gain': # gain function output (.mat).
			...
			if args.out_type is 'y': # enahnced speech output (.wav).
