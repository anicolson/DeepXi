...
## BRIEF:          Feature and target generation for polar representation in the acoustic-domain and the polar representation in the modulation-domain.
##
## This Source Code Form is subject to the terms of the Mozilla Public
## License, v. 2.0. If a copy of the MPL was not distributed with this
## file, You can obtain one at http://mozilla.org/MPL/2.0/.

from dev.modulation.analysis_synthesis import polar_polar
...
def input(z, z_len, N_w, N_s, NFFT, K_w, K_s, NFFT_m, f_s):
	'''
    Input features for polar form acoustic-domain and polar form modulation-domain.
	...
	Output/s:
		z_MAG_MAG - speech MM spectrum.
		z_MAG_PHA - speech MP spectrum.
	...
	z_MAG_MAG, z_MAG_PHA, z_PHA = polar_polar.analysis(z, N_w, N_s, NFFT, K_w, K_s, NFFT_m)
	return tf.expand_dims(z_MAG_MAG, axis=-1), Eta, z_MAG_PHA, z_PHA

def input_target_spec(s, d, s_len, d_len, SNR, N_w, N_s, NFFT, K_w, K_s, NFFT_m, f_s):
	'''
    Input features and target (spectrum) for polar form acoustic-domain and polar form modulation-domain.
	...
	Outputs:
		x_MAG_MAG - noisy speech MM spectrum.
		s_MAG_MAG - clean speech MM spectrum (target).
		Eta - number of acoustic-domain frames for each sequence.
	'''
	...
	x_MAG_MAG, _, _ = polar_polar.analysis(x, N_w, N_s, NFFT, K_w, K_s, NFFT_m)	
	x_MAG_MAG = tf.expand_dims(x_MAG_MAG, axis=-1)
	s_MAG_MAG, _, _ = polar_polar.analysis(s, N_w, N_s, NFFT, K_w, K_s, NFFT_m)
	s_MAG_MAG = tf.boolean_mask(tf.expand_dims(s_MAG_MAG, axis=-1), tf.sequence_mask(Eta))
	return x_MAG_MAG, s_MAG_MAG, Eta

def input_target_xi(s, d, s_len, d_len, SNR, N_w, N_s, NFFT, K_w, K_s, NFFT_m, f_s, mu, sigma):
	'''
    Input features and target (mapped a priori SNR) for polar form acoustic-domain and polar form modulation-domain.
	...
	s_MAG_MAG, _, _ = polar_polar.analysis(s, N_w, N_s, NFFT, K_w, K_s, NFFT_m)
	s_MAG_MAG = tf.boolean_mask(tf.expand_dims(s_MAG_MAG, axis=-1), tf.sequence_mask(Eta))
	d_MAG_MAG, _, _ = polar_polar.analysis(d, N_w, N_s, NFFT, K_w, K_s, NFFT_m)
	d_MAG_MAG = tf.boolean_mask(tf.expand_dims(d_MAG_MAG, axis=-1), tf.sequence_mask(Eta))
	...
	return x_MAG_MAG, xi_mapped, Eta

def target_xi(s, d, s_len, d_len, SNR, N_w, N_s, NFFT, K_w, K_s, NFFT_m, f_s):
	'''
    Target (a priori SNR) for polar form acoustic-domain and polar form modulation-domain.
