...
def BottlekneckBlock(x, norm_type, mask, training, d_model, d_f, k_size, d_rate):
	'''
	...
	layer_1 = CausalDilatedConv1d(tf.nn.relu(Normalisation(x, norm_type, mask, 
		training)), d_f, 1, 1, False)
	layer_2 = CausalDilatedConv1d(tf.nn.relu(Normalisation(layer_1, norm_type, 
		mask, training)), d_f, k_size, d_rate, False)
	layer_3 = CausalDilatedConv1d(tf.nn.relu(Normalisation(layer_2, norm_type, 
		mask, training)), d_model, 1, 1, True)
	return tf.add(x, layer_3)

def ResNet(x, seq_len, norm_type='layer', training=None, d_out=257, 
	...
	blocks = [tf.nn.relu(Normalisation(tf.layers.dense(x, 
		d_model, use_bias=False), norm_type, mask, training))] # (W -> Norm -> ReLU).
	for i in range(n_blocks): blocks.append(BottlekneckBlock(blocks[-1], norm_type, mask, training, 
